...
const PlayerInformation = (props) => {
    ...
    const advanceStatsDetails = () => {
        const playerId = props.location.state.playerId;
        const payload = new FormData();
        payload.append('player_id', playerId);
        axios.post('/StageGoalyApi/playersAdvanceStats', payload)
            .then(res => {
                if (res.data && res.data.success && res.data.error === 0) {
                    // console.log(res.data.players[0]);
    ...
    return (
        <React.Fragment>
            ...
            {tab === 'stats' && <PlayerStats playerInfo={playerInfo} advanceStats={advanceStats} loading={loading} />}
